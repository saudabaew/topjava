...
public class UserMealsUtil {
    public static void main(String[] args) {
        List<UserMeal> mealList = Arrays.asList(
                new UserMeal(LocalDateTime.of(2015, Month.MAY, 30, 20, 0), "Ужин", 430),
        ...
        getFilteredWithExceeded(mealList, LocalTime.of(7, 0), LocalTime.of(20, 1), 2000);
//        .toLocalDate();
//        .toLocalTime();
    }
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        mealList.stream()
                ...
                .map(userMeal -> userMealWithExceedList.add(new UserMealWithExceed(userMeal.getDateTime(),
                        userMeal.getDescription(),
                        userMeal.getCalories(),
                        map.get(userMeal.getDateTime().toLocalDate()) > caloriesPerDay)))
                .collect(Collectors.toList());

        userMealWithExceedList.sort(Comparator.comparing(UserMealWithExceed::getDateTime));
